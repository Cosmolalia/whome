        ...
        with self.canvas:
            ...
            for i in range(completed):
                ...
                if i >= completed - 3:
                    ...
                    Color(0, 0.55, 0.7, 0.85)  # muted cyan
                Rectangle(pos=(x + gap, y + gap), size=(s - gap * 2, s - gap * 2))

            # Current computing cell (gold pulse)
            if completed < total:
                x, y, s = self._cells[completed]
                gap = max(1, s * 0.08)
                Color(1, 0.84, 0, 0.9)  # gold
                Rectangle(pos=(x + gap, y + gap), size=(s - gap * 2, s - gap * 2))

            # Hits overlay (gold flash on random completed cells)
            # Just a subtle gold border on the first few completed cells
            if jobs_done > 0:
                Color(1, 0.84, 0, 0.3)
                for i in range(min(3, completed)):
                    x, y, s = self._cells[i]
...
class ComputeScreen(Screen):
    ...
    def start_work(self, api_key, worker_id, name, server):
        ...
        if self._service_mode:
            ...
            self._log('Worker started (in-process mode)')
            self._acquire_wakelock()
            threading.Thread(target=self._heartbeat_loop, daemon=True).start()
            threading.Thread(target=self._compute_loop, daemon=True).start()
    ...
    def _show_persistent_notification(self, text='Computing...'):
        """Show a foreground-style notification from the main process."""
        try:
            from jnius import autoclass
            from android import mActivity
    ...
    def _compute_loop(self):
        ...
        while self.running:
            try:
                ...
                if hasattr(self, '_notification_manager') and self._notification_manager:
                    if hits:
                        self._update_notification(f'HIT! Jobs: {self.jobs_done} | Hits: {self.discoveries}')
                    else:
    ...
    def _run_job(self, params):
        """Run eigenvalue computation with stage updates (in-process fallback)."""
        k = params['k']
...
class WAtHomeApp(App):
    ...
    def on_stop(self):
        # App closing — stop the service if running
        try:
