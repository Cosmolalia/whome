...
def run_job(params):
    ...
    if job_type == 'falsification':
        return run_falsification(params)
    elif job_type == 'boundary':
        return run_boundary(params)
    else:
        return run_eigenvalue(params)


def run_eigenvalue(params):
    k = params['k']
...
def compute_loop(cfg, manager):
    ...
    while True:
        ...
        # Max jobs check — sleep and re-check (user may change setting)
        if max_jobs > 0 and jobs_done >= max_jobs:
            write_status({
                ...
                "discoveries": discoveries,
                "compute_hours": round(compute_hours, 3),
                "message": f"Limit reached ({max_jobs}) — change in Settings to resume",
            })
            update_notification(manager, f"Paused — {jobs_done}/{max_jobs} jobs",
                               jobs=jobs_done, hits=discoveries, hours=compute_hours)
            time.sleep(10)
            continue
...
def main():
    ...
    # The foreground notification keeps the process alive, but WITHOUT a
    # wake lock the CPU sleeps and computation stops on screen lock.
    wake_lock = None
    try:
        from jnius import autoclass
        Context = autoclass('android.content.Context')
        PowerManager = autoclass('android.os.PowerManager')
        pm = _service_ctx.getSystemService(Context.POWER_SERVICE)
        wake_lock = pm.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "wathome:compute")
        wake_lock.acquire()
        write_status({"state": "init", "message": "Wake lock acquired"})
    except Exception as e:
        write_status({"state": "init", "message": f"Wake lock failed: {e}"})
    ...
    try:
        ...
        if notif:
            service_cls.setAutoRestartService(True)
            # Android 14+ (API 34) requires foreground service type.
            # Use 3-arg startForeground on API 29+ for compatibility.
            try:
                Build = autoclass('android.os.Build$VERSION')
                if Build.SDK_INT >= 29:
                    ServiceInfo = autoclass('android.content.pm.ServiceInfo')
                    _service_ctx.startForeground(
                        notification_id, notif,
                        ServiceInfo.FOREGROUND_SERVICE_TYPE_DATA_SYNC)
                    write_status({"state": "init", "message": "Foreground started (dataSync type)"})
                else:
                    _service_ctx.startForeground(notification_id, notif)
                    write_status({"state": "init", "message": "Foreground started (legacy)"})
            except Exception as e2:
                # Fallback to 2-arg if type constant unavailable
                _service_ctx.startForeground(notification_id, notif)
                write_status({"state": "init", "message": f"Foreground started (2-arg fallback: {e2})"})
        else:
    ...
    try:
        ...
        # Release wake lock so CPU can sleep after service stops
